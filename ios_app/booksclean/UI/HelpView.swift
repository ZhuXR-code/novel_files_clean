    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                group("4. 其他功能", items: [
                    "「关键词替换规则」：在扫描/解析阶段对文件名与字段做精确字符串替换（如去除水印、推广语）。支持「批量新增」：一次粘贴多行，两种模式任选——「去掉关键词」每行一个关键词（整行内容被移除），「替换关键词」每行用 || 分隔（如 AAA||B 表示把 AAA 替换成 B）。",
                    "「导出已标记文件清单」：在「设置 → 数据与备份」中把已标记（标星）的文件清单导出为文本文件，便于备份待删列表；无标记文件时会提示。",
