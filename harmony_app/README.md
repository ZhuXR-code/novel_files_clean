...
### 2026-07-24 同步安卓端最近改动

- `pages/LibraryScreen.ets`：底部操作栏简化，只保留「清空勾选」和「删除」两个按钮，其余按钮（规则标重/书名标重/清空标记/导出勾选/导出标记）移除，与安卓端保持一致。
- `pages/Help.ets`：帮助文档升级，新增文件内容预览（竖向/横向滚动条）、分页浏览、拼音搜索、阅读设置等详细说明，与安卓端帮助文档对齐。
