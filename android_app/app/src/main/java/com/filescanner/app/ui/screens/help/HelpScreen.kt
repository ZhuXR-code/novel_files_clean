...
fun HelpScreen(onBack: () -> Unit) {
    val items = listOf(
        "1. 开始扫描" to "在首页点击「开始扫描」，进入“扫描配置”列表：可新增配置（选择文件夹、文件类型、是否递归、排除文件夹），或从已有配置直接开始。应用会递归读取文件、并在扫描时同步解析文件名提取书名/作者/进度/来源，用于识别重复。扫描结果保存在本机，不会上传。",
        "2. 查看文库与筛选" to "扫描完成后进入文库，会按“每次扫描”分出多个文库。在文库内可按「全部 / 已勾选 / 未勾选 / 已标记 / 未标记」筛选，按时间、名称、大小排序，并支持搜索。列表/合集两种视图通过顶部分段开关切换。",
        ...
        "5. 标记重复与勾选" to "合集模式下点击右上角菜单「标记重复」：在「同一文库内、按 (作者 + 小说名) 子分组」套用与 PC 端 backend/dup_logic.py 完全一致的五则规则智能勾选待删重复项——① 完全相等去重：小说名+作者+大小+进度 四字段完全一致的多本中（不再比较文件名），保留最新(创建最晚，并列取 id 最大) 一本，其余全部勾选；② 纯数字进度对比：同组内所有进度均为纯数字时，进度数字最大者不勾选，其余纯数字文件全部勾选；③ 含中文进度 / 完结特例：进度含中文(如完结/连载/断更) 不勾选，但若同组存在文件名带「完结/完本/全本/全集/完整/全套/全集版」等关键词、且「进度数字最大文件」的大小小于同组所有含中文进度文件的大小，则该进度最大文件也要勾选（存在更完整的完结版，部分进度版冗余应删）；④ 最大文件不勾选原则：已勾选文件中，本 (作者+小说名) 组内唯一文件大小最大者不勾选（并列最大时不据此保护，以免同尺寸重复组被整体保留）；⑤ 完结+N番外 组合排序：进度【严格】匹配「完结+数字番外」(如「完结+3番外」) 的文件，在同 (作者+小说名) 组内按数字 N 排序，数字最大者不勾选、其余勾选；但若被勾选的文件恰为本组 (作者+小说名) 内文件大小最大者，则也不勾选。示例：F1 进度 80%(2.0MB)、F2「斗破苍穹完结版.txt」进度 完结(5.0MB)、F3 进度 完结(6.0MB) → F2/F3 含中文受保护保留，F1 进度最大但比所有含中文文件都小且有完结关键词 → F1 强制勾选(删)，F2/F3 保留。也可「按书名/作者相同标记」给文件加星标、「清除标记」取消。星标（已标记）会贯穿筛选与导出。注意：「标记重复」写入的是【勾选(checked)】而非星标，勾选会持久保存，可在顶部筛选选「已勾选 / 未勾选」聚焦待删项；行尾星标仅供手动标记，与「标记重复」无关。",
        "6. 选择并删除" to "勾选要删除的文件（合集模式下「标记重复」会自动勾选重复项），底部出现「批量删除选中」按钮，点击先让你选择【仅删除记录】或【删除记录和源文件(不可恢复)】，确认后由前台服务逐个删除并实时显示结果；删除前在确认页仍可取消勾选。",
