package com.booksclean.app.ui.screens.help
...
fun HelpScreen(onBack: () -> Unit) {
    val items = listOf(
        ...
        "8. 设置" to "在「设置」中可调整主题与全局字号（自动适配不同手机屏幕）；「阅读设置」可切换预览页的竖向/横向滚动条模式；配置「关键词替换」规则做文件名数据清洗（扫描阶段清洗文件名、解析阶段清洗书名/作者等）；「勾选重复规则」可分别开关六条去重规则（默认全部启用），关闭后「勾选重复」和删除操作不再使用该规则判断；支持导出已标记清单与清空本地数据。",
