package com.bookscleanandroid.app.ui.screens.help
...
fun HelpScreen(onBack: () -> Unit) {
    val items = listOf(
        ...
        "2. 查看文库与筛选" to "扫描完成后进入文库，会按「每次扫描」分出多个文库。在文库内可按「全部 / 已勾选 / 未勾选 / 已标记 / 未标记」筛选，按时间、名称、大小排序。列表/合集两种视图通过顶部分段开关切换。\n\n支持「导出书籍列表」：右上角菜单 →「导出列表」，可把当前文库的书籍列表导出为 TXT 文件。导出前可勾选要包含的列（如小说名 / 作者 / 文件名 / 大小 / 进度 / 来源等，默认勾选小说名），并选择范围：「当前页」只导出当前分页，或「全量」导出当前筛选条件下的全部数据（复用与页面一致的筛选、搜索、排序口径，不会漏项）。导出后通过系统「另存为」选择保存位置，方便归档或在电脑上继续整理。",
        "2.1 搜索与拼音搜索" to "支持按书名、作者、文件名搜索。更强大的拼音搜索：输入拼音全拼（如 doupo）或首字母缩写（如 dpcq），即可搜到「斗破苍穹」等中文书名/作者名，不用切换到中文输入法即可快速定位目标。",
        "2.2 文件内容预览" to "在文库列表或文件详情点击文件即可进入「文件预览」，直接查看 txt 文本内容。预览页提供竖向（默认）和横向（顶部）两种滚动条模式，可在「设置 → 阅读设置」中切换。竖向模式下，右侧可拖动的滚动条，拖动滑块即可快速上下滑动浏览；横向模式下，顶部粗进度条可左右拖动快速定位阅读进度。触摸滚动条时会临时加粗方便操作，0.5 秒不操作恢复原样。内容较多时应用采用流式分批加载（默认仅读取前若干行，向下滚动到底部自动加载更多），并能自动识别 GBK / UTF-8 编码，避免大文件卡顿；滚动条仅在内容超出一屏时出现。",
        ...
        "8. 设置" to "在「设置」中可调整主题与全局字号（自动适配不同手机屏幕）；「阅读设置」可切换预览页的竖向/横向滚动条模式；配置「关键词替换」规则做文件名数据清洗（扫描阶段清洗文件名、解析阶段清洗书名/作者等），支持**批量新增**：点「批量新增」一次粘贴多行，两种模式任选——「去掉关键词」每行一个关键词（整行内容被移除），「替换关键词」每行用 `||` 分隔（如 `AAA||B` 表示把 AAA 替换成 B）；「勾选重复规则」可分别开关六条去重规则（默认全部启用），关闭后「勾选重复」和删除操作不再使用该规则判断；支持导出已标记清单、导出书籍列表与清空本地数据。",
