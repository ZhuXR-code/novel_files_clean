...
fun HelpScreen(onBack: () -> Unit) {
    val items = listOf(
        ...
        "10. 文件备注" to "在文件详情页可给单个文件添加「备注」，用于记录该文件的补充说明、待办或判重依据，不影响扫描解析、星标与勾选删除。同一文件可添加多条备注，每条 1~50 字；同文件内备注内容区分大小写去重（如「A」与「a」视为不同备注），重复内容会被自动忽略。在备注区点击「添加备注」填写内容，已添加的备注可随时「编辑」或「删除」。备注仅保存在本机数据库，删除文件记录时其备注一并清除；通过「合并文库」生成新文库时，原文件的备注会按内容自动复制并去重，不会丢失。",
        "11. 合并文库" to "在「文库」列表长按进入多选模式，勾选至少 2 个文库后，底部出现「合并」按钮，点击后输入新文库名称即可将所选文库合并为一个新文库。合并会生成独立的全新文库，原文库保持不变；文件按「书名 + 作者 + 路径」去重合并，被合并文库中已有的勾选状态、星标标记会一并带入新文库。合并前已添加的「文件备注」也会按内容自动复制并去重（同名文件同内容备注不重复），不会丢失。合并适合把多次扫描、分目录的结果汇总到一处统一查看与判重。"
    )
