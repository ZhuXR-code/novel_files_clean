...
- **效果**：生成可供查阅 / 归档的文件，列含 `文件名 / 小说名 / 作者 / 进度 / 来源 / 摘要`，即当前书籍列表的快照（含文件名，便于与磁盘文件一一对应）。
- **注意**：导出的是数据库里的解析结果，**不影响源文件**。导出的「书籍列表」可随时在 Excel / 文本编辑器中打开查看或归档。
...
- **操作**：切到「设置」页，分别维护：
  - **关键词替换**：「扫描阶段」和「解析阶段」的替换规则（`查找 → 替换为`，支持多条按顺序排列）。除逐条「新增」外，还支持**批量新增**：一次粘贴多行规则，两种模式任选——
    - **去掉关键词**：每行一个关键词，作为「删除替换」处理（整行内容将被移除）；
    - **替换关键词**：每行一条，用 `||` 分隔「被替换词」和「替换成词」，如 `AAA||B` 表示把 AAA 替换成 B。
...
- **注意**：规则按 `sort_order` 顺序生效；批量新增的规则自动追加到当前阶段末尾，可再手动拖动排序。改完规则后，建议对相关配置「强制重跑」对应解析以套用新规则。勾选重复规则关闭后立即生效，不影响数据库已有数据。
...
- 导出的即是当前**书籍列表**的快照（含文件名，可与磁盘文件一一对应），便于归档、备份或在 Excel / 文本编辑器中继续整理；导出仅读取数据库解析结果，不影响源文件。
...
**支持批量新增**：点击「批量新增」弹窗，可一次粘贴多行规则快速建立，两种模式任选：
- **去掉关键词**：每行一个关键词，作为「删除替换」处理（整行内容将被移除），适合批量去除水印、推广语等；
- **替换关键词**：每行一条，用 `||` 分隔「被替换词」和「替换成词」，如 `AAA||B` 表示把 AAA 替换成 B。
批量新增的规则自动追加到当前阶段（扫描 / 解析）末尾，可再手动拖动排序或逐条编辑。
